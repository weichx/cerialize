...
import { IConstructable, isPrimitiveType, SerializableType, SerializeFn, setBitConditionally } from "./util";
...
export function serializeAsArray(type : SerializableType<any>, keyName : string = null) {
...
export function serializeAsMap(type : SerializableType<any>, keyName : string = null) {
  return function (target : any, actualKeyName : string) : any {
    const metadata = MetaData.getMetaData(target.constructor, actualKeyName);
    metadata.serializedKey = (keyName) ? keyName : actualKeyName;
    metadata.serializedType = type;
...
export function serializeAsJson(keyName : string = null, transformKeys = true) {
  return function (target : IConstructable, actualKeyName : string) : void {
    const metadata = MetaData.getMetaData(target.constructor, actualKeyName);
    metadata.serializedKey = (keyName) ? keyName : actualKeyName;
    metadata.flags |= (MetaDataFlag.SerializeJSON);
    metadata.flags = setBitConditionally(metadata.flags, MetaDataFlag.SerializeJSONTransformKeys, transformKeys);
  };
}
...
export function autoserializeUsing(serializer : ISerializer, keyName? : string) {
  return function (target : IConstructable, actualKeyName : string) : void {
    const metadata = MetaData.getMetaData(target.constructor, actualKeyName);
...
export function autoserializeAsJson(keyName : string, transformKeys = true) {
  return function (target : IConstructable, actualKeyName : string) : void {
    const metadata = MetaData.getMetaData(target.constructor, actualKeyName);
    const key = (keyName) ? keyName : actualKeyName;
    metadata.deserializedKey = key;
    ...
    metadata.flags = setBitConditionally(metadata.flags, MetaDataFlag.AutoJSONTransformKeys, transformKeys);
  };
}
